...
GAME8 = 'Game2_10x10_Dynamic_Walls' # comm_atoc_eval_stuck_collision_v2_matrix_obs (no 2 means rmsprop)
GAME9 = 'Game2_10x10_Dynamic_Walls' # comm_atoc_eval_stuck_collision_v2_matrix_obs_ids
GAME10 = 'Game2_10x10_Dynamic_Walls' # comm_atoc_eval_stuck_collision_v2_matrix_obs_2 (2 means adam)
GAME11 = 'Game2_10x10_Dynamic_Walls' # comm_atoc_eval_stuck_collision_v3_binary_matrices_obs
GAME12 = 'Game2_10x10_Dynamic_Walls' # comm_atoc_eval_stuck_collision_v3
GAME13 = "Game2_10x10_Dynamic_Walls" # comm_atoc_eval_stuck_collision_v3_matrix_obs (fix the signal when agent reaches goal + multiply input by 100 + using latest backup controller)
...
one = np.ones(1000)
one.fill(0.9)
plt.plot(one, 'k', label="0.9")
plt.plot(np.array(game6_testing_cumulative_rewards).mean(axis=1), 'b', label='latest 3 pos shared + backup cont.')
...
#####plt.plot(np.array(game5_testing_cumulative_rewards).mean(axis=1), 'g', label='baseline with backup cont.')
#plt.plot(np.array(game7_testing_cumulative_rewards).mean(axis=1), 'r', label='baseline + backup cont. 2')

plt.plot(np.array(game14_testing_cumulative_rewards).mean(axis=1), '#7982c9', label='basline + numeric + backup cont. 3')
plt.plot(np.array(game12_testing_cumulative_rewards).mean(axis=1), '#9F0FD6', label='baseline + binary + backup cont. 3')
plt.plot(np.array(game13_testing_cumulative_rewards).mean(axis=1), '#d4177f', label='basline + CNN Adam + numeric + backup cont. 3')
plt.plot(np.array(game11_testing_cumulative_rewards).mean(axis=1), '#9c7722', label='basline + CNN Adam + binary + backup cont. 3')

#plt.plot(np.array(game8_testing_cumulative_rewards).mean(axis=1), '#7dd147', label='basline + CNN RMS + numeric + backup cont. 2')
######plt.plot(np.array(game9_testing_cumulative_rewards).mean(axis=1), 'b', label='basline + CNN (w ids) + backup cont. 2')
#plt.plot(np.array(game10_testing_cumulative_rewards).mean(axis=1), '#ff8e2e', label='basline + CNN Adam + numeric + backup cont. 2')
